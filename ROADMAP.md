...
- [ ] **B-6：弹窗关闭优化** - 支持遮罩点击 + Esc 键关闭
- [ ] **O-6：移动端触摸区域优化** - 大门/窗户触摸区域 ≥44×44px
- [ ] **内容迁移** - 四个房间内容充实（书房、创作室、会客厅、工作室）
- [ ] **根目录迁移** - 将 doghouse 移动到网站根目录 `/`
- [ ] **SEO 基础优化** - meta description、title 优化

### 中期（v0.3.0）

- [ ] **提取 `assets/css/room-base.css`**：消除四个房间页面的重复 CSS（O-1）
- [ ] **将弹窗样式从 JS 移入 `style.css`**：删除 `getModalStyles()` 函数（O-2）
- [ ] **弹窗支持遮罩点击 + Esc 键关闭**（B-6）
- [ ] **添加中文字体栈**（O-3）
- [ ] **CSS 动画性能优化**：`will-change: transform`（O-4）
...
| 日期 | 任务 | 产出 |
|------|------|------|
| **Day 1** (4.1) | Bug 修复 + 内容整理 | 弹窗/触摸优化完成 |
| **Day 2** (4.2) | 书房 + 创作室内容迁移 | 两个房间充实 |
| **Day 3** (4.3) | 会客厅 + 工作室内容迁移 | 四个房间都有内容 |
| **Day 4** (4.4) | **根目录迁移** | doghouse 成为主页 |
| **Day 5** (4.5) | SEO + 测试 | 桌面/移动端测试通过 |
| **Day 6** (4.6) | 缓冲 + 修复 | 问题修复 |
| **Day 7** (4.7) | 发布 + 文档 | v0.2.0 发布！🎉 |

**核心目标**：4 月 7 日前 doghouse 上线成为主页（http://118.145.223.142/）
...
| v0.2.0 | 2026-04-07（计划） | Bug 修复 + 内容迁移 + 根目录迁移 |
